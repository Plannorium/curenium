...
const DashboardContent: React.FC = () => {
  ...
  // Filter appointments - pending are those in the future that are not cancelled/completed
  const pendingAppointments = appointments?.filter(appointment =>
    new Date(appointment.date) > new Date() &&
    !['completed', 'cancelled'].includes(appointment.status)
  ).length || 0;

  // Overdue appointments are past appointments that are not completed or cancelled
