  ...
  ({ ward, onWardUpdated, children }) => {
    ...
    useEffect(() => {
      if (ward && isOpen) {
        ...
        setPhone(ward.contactInfo?.phone || "");
        setExtension(ward.contactInfo?.extension || "");
      }
    }, [ward, isOpen]);
    ...
    const handleSubmit = async () => {
      if (!ward || !name.trim() || !wardNumber.trim() || !department || !totalBeds || !wardType) return;

      setIsSubmitting(true);
      try {
        const res = await fetch(`/api/wards/${ward._id}`, {
          method: "PUT",
...
EditWardModal.displayName = "EditWardModal";
