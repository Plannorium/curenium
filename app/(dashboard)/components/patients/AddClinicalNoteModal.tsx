...
export function AddClinicalNoteModal({ patientId, isOpen, onClose, onNoteAdded }: AddClinicalNoteModalProps) {
  ...
  const handleSubmit = async () => {
    if (!content) {
      toast.error(t('addClinicalNoteModal.noteContentRequired'));
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        toast.success(t('addClinicalNoteModal.clinicalNoteAdded'));
        onNoteAdded();
        onClose();
      } else {
        toast.error(t('addClinicalNoteModal.failedToAddClinicalNote'));
      }
    } catch (error) {
      toast.error(t('addClinicalNoteModal.errorAddingClinicalNote'));
    }
  };
        ...
        <DialogHeader className="pb-4">
          <div className="flex items-center space-x-3 mb-2">
            ...
            <DialogTitle className="text-xl font-bold bg-linear-to-r from-gray-900 via-gray-800 to-gray-900 dark:from-white dark:via-gray-100 dark:to-white bg-clip-text text-transparent">
              {t('addClinicalNoteModal.title')}
            </DialogTitle>
          </div>
          <p className="text-sm text-muted-foreground">{t('addClinicalNoteModal.subtitle')}</p>
        </DialogHeader>
        <form onSubmit={(e) => {
          e.preventDefault();
          handleSubmit();
          ...
          <div className="space-y-3">
            <label htmlFor="note-content" className="text-sm font-semibold text-gray-800 dark:text-gray-200 flex items-center">
              ...
              {t('addClinicalNoteModal.noteContent')}
            </label>
            <Textarea
              id="note-content"
              placeholder={t('addClinicalNoteModal.noteContentPlaceholder')}
              value={content}
          ...
          <div className="space-y-3">
            <label htmlFor="visibility" className="text-sm font-semibold text-gray-800 dark:text-gray-200 flex items-center">
              ...
              {t('addClinicalNoteModal.visibility')}
            </label>
            <Select onValueChange={(value: 'team' | 'private' | 'public') => setVisibility(value)} defaultValue={visibility}>
              <SelectTrigger id="visibility" className="bg-gray-50/80 dark:bg-gray-900/80 border-gray-200/70 dark:border-gray-700/60 focus:border-blue-400 dark:focus:border-blue-500 rounded-xl h-11">
                <SelectValue placeholder={t('addClinicalNoteModal.selectVisibility')} />
              </SelectTrigger>
              <SelectContent className="bg-white/90 dark:bg-gray-900/90 backdrop-blur-lg border border-gray-200/50 dark:border-gray-700/50 rounded-xl">
                <SelectItem value="team" className="hover:bg-blue-50 dark:hover:bg-blue-950/30 rounded-lg mx-1">{t('addClinicalNoteModal.team')}</SelectItem>
                <SelectItem value="private" className="hover:bg-orange-50 dark:hover:bg-orange-950/30 rounded-lg mx-1">{t('addClinicalNoteModal.private')}</SelectItem>
                <SelectItem value="public" className="hover:bg-green-50 dark:hover:bg-green-950/30 rounded-lg mx-1">{t('addClinicalNoteModal.public')}</SelectItem>
              </SelectContent>
            </Select>
          </div>
          <DialogFooter className="pt-4 border-t border-gray-200/50 dark:border-gray-700/50">
            ...
            >
              {t('addClinicalNoteModal.saveNote')}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
