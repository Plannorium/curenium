  ...
  const handleMemberAction = async (userId: string, action: 'add' | 'remove') => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.error(`Failed to ${action} member`);
      }
    } catch (error) {
      console.error(`An error occurred while trying to ${action} a member:`, error);
    } finally {
      setIsUpdating(null);
    }
  };
      ...
      <DialogContent className="backdrop-blur-xl bg-background/95 border-border/50 shadow-2xl max-w-lg">
        ...
        <div className="py-4 space-y-6 max-h-[70vh] flex flex-col">
              ...
              <div className="max-h-32 overflow-y-auto space-y-1 custom-scrollbar pr-2">
                {usersToAdd.map(user => (
                  <div key={user._id} className="flex items-center justify-between p-2 rounded-lg hover:bg-accent/50">
                    ...
                    <Button size="sm" variant="ghost" onClick={() => handleMemberAction(user._id, 'add')} disabled={isUpdating === user._id}>
                      {isUpdating === user._id ? <Loader2 size={16} className="animate-spin" /> : <UserPlus size={16} />}
                    </Button>
                  </div>
                ))}
                {usersToAdd.length === 0 && <p className="text-xs text-muted-foreground text-center p-2">No users found.</p>}
              </div>
            )}
          </div>
          ...
          <div className="space-y-3 flex-1 min-h-0">
            <h3 className="text-sm font-semibold text-foreground">{channelMembers.length} Member(s)</h3>
            <div className="max-h-full overflow-y-auto space-y-1 custom-scrollbar pr-2">
              {channelMembers.map(member => (
                <div key={member._id} className="flex items-center justify-between p-2 rounded-lg hover:bg-accent/50">
                  ...
                  <Button size="sm" variant="ghost" className="text-red-500 hover:text-red-600" onClick={() => handleMemberAction(member._id, 'remove')} disabled={isUpdating === member._id}>
