...
const Call: React.FC<CallProps> = ({ callId, localStream, onInvite, remoteStreams, onEndCall, userName, onToggleScreenShare, isScreenSharing, screenStream, isMuted, isVideoOff, onToggleMute, onToggleVideo }) => {
  ...
  }, [localStream, isMuted, onToggleMute, playSoundWithInit]);
  ...
  }, [localStream, isVideoOff, onToggleVideo, playSoundWithInit]);
  ...
  const handleGesture = useCallback(
    ...
    [handleToggleMute, handleToggleVideo, onEndCall, playSoundWithInit]
  );
      ...
      <AnimatePresence>
        {isChatOpen && (
          <motion.div initial={{ x: '100%' }} animate={{ x: '0%' }} exit={{ x: '100%' }} transition={{ type: 'spring', stiffness: 300, damping: 30 }} className="absolute top-1 right-1 bottom-1 flex h-full">
            {isMobile ? (
              <div className="h-full w-fit bg-transparent flex flex-col">
                {chatPanel}
              </div>
            ) : (
              <ResizableBox
